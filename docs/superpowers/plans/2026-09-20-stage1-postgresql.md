...
| Docker | не установлен, демон недоступен | `docker-compose.yml` — артефакт без локального smoke-теста; это указывается явно |
...
**Замечание об ограничении проверки:** на машине разработки, где писался этот план,
Docker не установлен. Файлы валидируются статически (`docker compose config` там, где
доступно) и помечаются как непроверенные запуском до первого прогона на машине с Docker.
Заявлять «окружение работает» без фактического `up` нельзя (ТЗ п.85 о неподтверждённых
утверждениях распространяется и на это).
