...
| Производительность на машине разработчика | **измерена**: ~150 RPS чтение, ~81 запись/с, см. `docs/PERFORMANCE.md` раздел 5 |
| Целостность данных после нагрузки | **проверена** после каждого прогона: VoteRecord == Ballot, 0 дублей, 0 orphan |
| Отсутствие deadlock'ов под нагрузкой | **проверено**: 0 до и после всех прогонов |
| Производительность на целевых 20–40k RPS | **не измерялась** — нужна инфраструктура, которой нет |
