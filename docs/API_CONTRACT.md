...
| `internal_server_error` | 500 | необработанное исключение; **`message` всё ещё содержит `str(exc)`** — D-06, общий обработчик чинится на этапе 7 |
...
```json
{
  ...
  "auth_version": <int, с этапа 3>,
  "exp": <unix, +7 дней>,
  "iat": <unix>
}
```

Распознаётся по `token_type == "student"`.

**Проверка токена (с этапа 3).** Подпись проверяется локально, затем личность
берётся из Redis (`voteplatform:v1:student:{id}:principal`). При попадании
в кэш обращений к базе **ноль**; при промахе — один `SELECT` и заполнение кэша.
Недоступный Redis не мешает: аутентификация идёт через PostgreSQL.

Порядок проверок: подпись → `is_active` → `auth_version`.

**`auth_version` и отзыв токенов.** Claim сверяется с версией студента.
Версия увеличивается при деактивации, смене пароля и явном отзыве, после чего
все ранее выданные токены немедленно перестают действовать: ответ **403**,
`"Сессия завершена. Войдите заново"`. Деактивированный студент получает
403 `"Учётная запись студента деактивирована"`.

**Обратная совместимость.** Токены без claim `auth_version`, выпущенные до
этапа 3, принимаются, пока включён `STUDENT_TOKEN_ALLOW_MISSING_AUTH_VERSION`
(по умолчанию `True`). Деактивированного студента это послабление не пропускает.
...
Исправлены на этапе 3: **D-04** (`is_active` и отзыв токенов),
**D-06 частично** (авторизация больше не отдаёт `str(exc)`).

Остаются: **D-05** (OTP в открытом виде) — этап 8;
**D-06** в части общего обработчика 500 — этап 7.
