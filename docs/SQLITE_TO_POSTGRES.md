...
## 9a. Индексы на большой таблице

Миграции проекта создают индексы через `apps/core/migration_operations.AddIndexSafely`,
который на PostgreSQL использует `CREATE INDEX CONCURRENTLY` (ТЗ п.54).

Почему это важно: обычный `CREATE INDEX` берёт на таблице блокировку SHARE.
Чтения проходят, а любая запись ждёт окончания построения — включая приём
голосов, потому что `VoteRecord` ссылается на студента внешним ключом.
На 200 000 студентов три индекса строятся 2.6 с, на миллионе — десятки секунд.

Следствия для эксплуатации:

- миграции с такими индексами объявляют `atomic = False`; это нормально;
- при сбое `CONCURRENTLY` остаётся **невалидный** индекс — его нужно удалить
  вручную, автоматически он не исчезнет:
...
psql -d vote_db -c "DROP INDEX CONCURRENTLY <имя>;"
```

- после `loaddata` и после перестройки индексов обязательно:

```bash
psql -d vote_db -c "VACUUM ANALYZE;"
```

Без этого Index Only Scan не работает: карта видимости пуста, и запросы
падают обратно на чтение кучи. Измерено: `COUNT` избирателей — 10.6 мс
до `VACUUM` и 0.88 мс после.
