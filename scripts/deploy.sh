...
# Настройка IP allowlist для Django Admin (ТЗ п.10)
...
deny all;
EOF
elif [ ! -f deploy/nginx/admin_ips.conf ]; then
    cat << 'EOF' > deploy/nginx/admin_ips.conf
# IP allowlist для доступа к Django Admin (ТЗ п.10)
# Задайте ADMIN_ALLOWED_IP в .env для автоматического ограничения.
EOF
fi

# Генерация https.conf из шаблона при наличии API_DOMAIN (ТЗ п.4)
