...
| [docs/LOAD_TESTING.md](docs/LOAD_TESTING.md) | как запускать нагрузочные сценарии |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | как система устроена и почему именно так |
| [docs/DEPLOY_PRODUCTION.md](docs/DEPLOY_PRODUCTION.md) | топология, развёртывание, откат |
| [docs/RUNBOOK.md](docs/RUNBOOK.md) | бэкапы, восстановление, откат релиза, инциденты |
| [docs/PROFILING.md](docs/PROFILING.md) | профилирование и вторая итерация оптимизации |
| [docs/REPORT_FINAL.md](docs/REPORT_FINAL.md) | итоговый отчёт и Definition of Done |
...
этап 10 (нагрузочное тестирование), этап 11 (профилирование и вторая итерация),
этап 12 (реплика чтения, бэкапы, отказоустойчивость, документация).

Все 13 этапов завершены. Итоги — [docs/REPORT_FINAL.md](docs/REPORT_FINAL.md).

Производительность измерена на машине разработчика (8 ядер, 3.8 ГБ Docker).
После второй итерации оптимизации: **285 durable записей голосов в секунду
при p95 174 мс и нуле ошибок 5xx**, 0 deadlock'ов, целостность данных
подтверждена после каждого прогона. До оптимизации на той же нагрузке
p95 составлял 364 мс. Полные результаты с зафиксированным окружением —
[docs/PERFORMANCE.md](docs/PERFORMANCE.md), метод измерения —
[docs/PROFILING.md](docs/PROFILING.md).

**Эти цифры характеризуют ноутбук, а не архитектуру, и не подтверждают
целевые 20–40k RPS.** Генератор нагрузки делит восемь ядер с приложением
и базой и сам берёт до 257 % CPU; объём тестовых данных ограничен его
памятью. Сценарии параметризуемы, включая ступени до 40k, и готовы
к запуску на настоящей инфраструктуре.
