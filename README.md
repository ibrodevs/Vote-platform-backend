...
этап 10 (нагрузочное тестирование).

Производительность измерена на машине разработчика (8 ядер, 3.8 ГБ Docker):
~150 RPS на чтение и ~81 durable запись голосов в секунду, 0 deadlock'ов,
целостность данных подтверждена после каждого прогона. Полные результаты
с зафиксированным окружением — в [docs/PERFORMANCE.md](docs/PERFORMANCE.md).

**Эти цифры характеризуют ноутбук, а не архитектуру, и не подтверждают
целевые 20–40k RPS.** Генератор нагрузки делит ядра с приложением и базой.
Сценарии параметризуемы и готовы к запуску на настоящей инфраструктуре.
